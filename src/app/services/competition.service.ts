...
export class CompetitionService {
  ...
  getCompetitions() {
    this.http.get<CompetitonList>(`${this.apiUrl}competitions`).subscribe(comps => {
  ...
  getMatches(competitionId: number, status: matchStatus): Observable<Match[]> {
    return this.http.get<Match[]>(`${this.apiUrl}competitions/${competitionId}/matches?status=${status}`);
  }

  getMatch(matchId: number): Observable<MatchList> {
    return this.http.get<MatchList>(`${this.apiUrl}matches/${matchId}`)
  }
}
